...
- 接收方首次选择一个接收文件夹，当前页面会话中的后续文件自动保存；同名文件使用 `(1)`、`(2)` 后缀避免覆盖
...
- 桌面 Chrome / Edge 首次接收时选择接收文件夹，当前页面会话的后续文件无需逐个确认。
- 文件默认保持原始文件名；如果目标目录已有同名文件，则自动生成 `文件名 (1).扩展名`，不静默覆盖已有文件。
- 不支持目录授权的浏览器继续使用逐文件保存对话框或小文件内存下载降级。
